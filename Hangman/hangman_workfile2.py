...
def checking_password(number):
    global hidden_password2, hidden_password
    if hidden_password2 == hidden_password:
        with open("points.txt", "r") as variables:        
            points = int(variables.readline())
            points += number
        with open("points.txt", "w") as variables:
            variables.write(str(points))
    time.sleep(1)
...
def play_game(user_name):
    global hidden_password, country
...
def init():
    global hidden_password, hidden_password2, country, used, start_time    
    hidden_password = []
    hidden_password2 = []
    country = ""
    used = []
