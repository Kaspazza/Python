...
# saving all variables in file
def reset_variables():
    with open("vars.txt", "w") as variables:        
        for var in range(6):
            variables.write("0;")  # points, hidden_password, hiden_password2, country, used, start_time


# reading variable from file
def read_variable(index):
    with open("vars.txt", "r") as variables:
        var = variables.readline()
        var = var.split(";")
        var = var[index]
        return var


# overwriting chosen variable
def write_variable(var_string, index):
    with open("vars.txt", "r+") as variables:
        var = variables.readline()
        var = var.split(";")
        var[index] = str(var_string)
        for v in var:
            variables.write(v + ";")
...
def printing_password(password, hidden_password):
    ...
    write_variable(hidden_password2, 2)


# Printing out penalty points and hint
def checking_password(number):
    # global points, hidden_password2, hidden_password
    points = int(read_variable(0))
    hidden_password2 = read_variable(1)
    hidden_password2 = read_variable(2)
    country = read_variable(3)
    if hidden_password2 == hidden_password:
        points += number
        write_variable(points, 0)
    time.sleep(1)
    print("\n Boo! You have +", number, "penalty points!")
    time.sleep(1)
    write_variable(hidden_password, 2)
    if points < 4:
...
def word(password, user_name):
    # print(password)
    start_time = int(read_variable(5))
...
def checking(password, user_name):
    # global hidden_password
    used = read_variable(4)
    hidden_password = read_variable(1)
    print_hidden(hidden_password)
    start_time = int(read_variable(5))
